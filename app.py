...
from src.pipline.prediction_pipeline import CustomerChurnData, CustomerChurnClassifier
...
class DataForm:
    ...
    async def get_customer_data(self):
        ...
        self.SeniorCitizen = int(form.get("SeniorCitizen")) if form.get("SeniorCitizen") else 0
        self.Partner = form.get("Partner")
        self.Dependents = form.get("Dependents")
        self.tenure = int(form.get("tenure")) if form.get("tenure") else 0
        ...
        self.MonthlyCharges = float(form.get("MonthlyCharges")) if form.get("MonthlyCharges") else 0.0
        self.TotalCharges = float(form.get("TotalCharges")) if form.get("TotalCharges") else 0.0
...
async def index(request: Request):
    """
    Renders the main HTML form page for customer data input.
    """
    # Modern Starlette (recommended, explicit keywords avoid ambiguity)
...
async def predictRouteClient(request: Request):
    ...
    try:
        ...
        # Interpret the prediction result as 'Customer will Churn' or 'Customer will NOT Churn'
